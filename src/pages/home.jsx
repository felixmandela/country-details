...
export default function Home({ country }) {
    ...
    useEffect(() => {
        ...
        const noFilter = filteredCountry.filter((c) =>
            regex.test(c.name.common)
        );
        const filter = filteredCountry
            .filter((c) => regex.test(c.name.common))
            .slice(0, 10);
        setSearchedCountry(region ? noFilter : filter);
    }, [search, filteredCountry, region]);

    return (
        <>
            <section className="mb-12 mt-12 flex flex-col gap-y-6 md:mt-0 md:flex-row md:items-center md:justify-between md:gap-y-0">
